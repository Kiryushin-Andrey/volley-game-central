...
**API:** replace global-admin-only guard on player-levels routes with **level steward** access (`isAdmin || isTc`) for `GET /player-levels/users` and `PATCH /player-levels/users/:userId`. Expose `isTc` on `/users/me`; continue omitting `playerLevel` from session for everyone.

**UI:** allow TC on `/player-levels` (list, name filter, assign level in dialog with immediate save). Toolbar **Players** for TC-only (`isTc && !isAdmin`) navigates directly to `/player-levels`, not the **Players hub**. Redirect TC-only users away from `/players`, `/game-administrators`, and priority-players admin routes. **Global administrators** keep existing hub behavior. **Assigned game administrators** without steward role remain blocked from player levels.
...
- [ ] Steward auth: TC and global admin can list/PATCH levels; participant and assigned-only admin receive 403
...
36–38, 44, 59; steward access for 29–30, 57 (assign only).
