...
function rowGameFormat(row: { gameFormat: string }): GameFormat {
  return row.gameFormat as GameFormat;
}

function gameRowForFormatHelpers(row: { dateTime: Date | string; gameFormat: string }) {
  return { dateTime: row.dateTime, gameFormat: rowGameFormat(row) };
}
  ...
  const priorityPlayerCheck = await db
    ...
    .where(
      and(
        ...
        eq(gameAdministrators.withPositions, withPositionsForAdminAssignment(game.gameFormat)),
...
router.post('/:gameId/register', async (req, res) => {
  try {
    ...
    const registrationOpenDays = await getRegistrationOpenDays(userId, gameRowForFormatHelpers(game[0]), isGuestRegistration);
    const playerLevel = (req.user.playerLevel as PlayerLevel | null) ?? null;
    const eligibility = evaluateRegistrationEligibility({
      gameFormat: rowGameFormat(game[0]),
      playerLevel,
      gameDateTime,
      now,
      isGuestRegistration,
    ...
    if (!eligibility.canSelfRegister) {
      if (eligibility.blockReason === 'level') {
        return res.status(403).json({
          error: 'You cannot register for this game.',
          gameDateTime,
      ...
      const errorMessage = isGuestRegistration
        ...
        : usesPriorityPlayerWindows(rowGameFormat(game[0]))
...
router.get('/:gameId', async (req, res) => {
  try {
    ...
    const registrationOpenDays = await getRegistrationOpenDays(req.user.id, gameRowForFormatHelpers(game[0]), false);
    const playerLevel = (req.user.playerLevel as PlayerLevel | null) ?? null;
    const now = new Date();
    const eligibility = evaluateRegistrationEligibility({
      gameFormat: rowGameFormat(game[0]),
  ...
  try {
          ...
          filteredGames = filteredGames.filter((game) => {
            ...
            return userAssignments.some(
              (assignment) =>
                ...
                assignment.withPositions === withPositionsForAdminAssignment(rowGameFormat(game))
            );
          });
        } else {
    ...
    if (categories && categories.length > 0 && !showPast) {
      ...
      if (validSelectedCategories.length > 0) {
        filteredGames = filteredGames.filter((game) => {
          const gameCategory = classifyGame(gameRowForFormatHelpers(game));
