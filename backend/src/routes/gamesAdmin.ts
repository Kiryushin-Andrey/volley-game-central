...
import { isValidGameFormat, type GameFormat } from '../domain/gameFormat';
...
router.get('/defaults', async (req, res) => {
  try {
    const { defaultDateTime, defaultLocationName, defaultLocationLink, defaultPaymentAmount, defaultPricingMode, defaultGameFormat } =
...
router.post('/', async (req, res) => {
  try {
    ...
    try {
      ...
      const isRecreational = created.gameFormat === 'recreational';
      const isFutureGame = gameDate > now;
      const isReadonly = !!created.readonly;

      if (isRegistrationOpen && isRecreational && isFutureGame && !isReadonly) {
