...
**Player levels page**:
The list of all registered users with **Level pill**s, name filter, **level multiselect** (Unassigned / Advanced / Intermediate / Beginner — same UX as the games home category filter; default all tiers selected), and row → **Player info dialog**. Level selection is OR across chosen tiers; name filter is AND with that result; client-side on the loaded list. Toolbar “Players” for **Technical Committee member**s who are not global administrators opens this page directly (`/player-levels`), not the **Players hub**. Data comes from the player-levels admin API: full list with client-side filter on this page; not embedded on game participant payloads.
...
- Player levels level filter — resolved: client-side **level multiselect** on **Player levels page** (category-filter pattern), combinable with name filter (PRD stories 31–32).
