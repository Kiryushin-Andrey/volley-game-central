...
- **Cursor Cloud / CI artifacts:** When `CI` is set, `CURSOR_ARTIFACTS=1`, or `/opt/cursor/artifacts` exists, Playwright writes screenshots and traces to `/opt/cursor/artifacts/e2e-test-results/` and the HTML report to `/opt/cursor/artifacts/playwright-report/`. Those paths are uploaded for viewing in the Cursor Cloud run UI. Local runs without those conditions keep the default `test-results/` and `playwright-report/` directories (gitignored). Override the root with `CURSOR_ARTIFACTS_DIR` if needed.
- **GitHub Pages (mobile-friendly report):** After a Cloud session run, publish all test screenshots via the `e2e-run-and-publish` skill or `npm run e2e:publish-report` (after `CI=true npm run test:e2e`). This pushes the Playwright HTML report to the `gh-pages` branch. **One-time setup:** GitHub → Settings → Pages → deploy from branch `gh-pages` / root. **Report URL:** https://kiryushin-andrey.github.io/volley-game-central/ — open in any mobile browser to browse every test’s screenshot.
